...
function CustomerOrderPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full p-3 md:p-4 pb-32">
        {isOrderSent ? (
          ...
          <>
            <div className="flex min-w-0 flex-row items-start gap-3 md:gap-5">
              <aside className="w-[130px] md:w-[150px] shrink-0 self-start">
                <div className="sticky top-[88px] h-[calc(100vh-108px)] rounded-3xl border border-slate-200 bg-white p-2 md:p-3 shadow-sm overflow-y-auto custom-scrollbar">
                  <div className="flex flex-col gap-2">
                  {MENU_CATEGORIES.map(cat => (
                    <button
                      key={cat}
                      onClick={() => setMenuActiveCategory(cat)}
                      className={`w-full px-3 py-3 rounded-2xl text-left text-xs md:text-sm leading-tight font-bold whitespace-normal break-words transition-all ${
                        menuActiveCategory === cat
              ...
              <section className="min-w-0 flex-1 grid grid-cols-1 gap-3 md:gap-4">
              {menuItems
                ...
                .map(item => {
                  ...
                  return (
                    <div key={item.id} className="min-w-0 bg-white p-2.5 sm:p-3 md:p-4 rounded-3xl border border-slate-200 flex gap-2.5 sm:gap-3 md:gap-4 shadow-sm hover:shadow-md transition-shadow">
                      <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden bg-slate-100 shrink-0">
                        <LazyImage 
                      ...
                      <div className="min-w-0 flex-1 flex flex-col justify-between py-1">
                        <div>
                          <h3 className="text-sm sm:text-base font-bold text-slate-800 break-words">{item.name}</h3>
                          {!isCustomerView && (
                        ...
                        <div className="flex justify-end items-center gap-3">
                          {totalQty > 0 ? (
                            <div className="max-w-full flex items-center gap-2 sm:gap-4 bg-slate-100 rounded-xl px-2 py-1">
                            ...
                            <button 
                              onClick={() => addToOrder(item)}
                              className="w-9 h-9 md:w-10 md:h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center shadow-md shadow-emerald-100 active:scale-90 transition-transform"
                            >
